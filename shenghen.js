window.func=function(lib,game,ui,get,ai,_status){
	var list=[];
	if(lib.config['extension_扩展ol_shenghen']!=true&&(lib.config.extension_扩展ol_8gEquips_choice!=true||lib.config.extension_扩展ol_8gEquips_choice1!=true)){
		list=[
			['heart',4,'SH_夏洛特1'],
			['heart',8,'SH_夏洛特2'],
			['heart',11,'SH_夏洛特3'],
	...
	for(var i=0;i<list.length;i++){
		var card=list[i];
		if(i>0) str+='、';
		str+=translate[card[2]]+'【'+get.translation(card[0])+card[1]+'】';
	};
	lib.extensionMenu['extension_扩展ol']['shenghen'].intro+='<br>该包添加的手牌：<br>'+str;
