			...
			var naihehualuo={
				...
				skill:{
					"_NHHL_turnOver":{
						...
						filter:function(event,player){
							for(var i in lib.characterPack['naihehualuo']){
								if(player.name==i&&!player.avatar2) return true;
							};
							return false;
						},
						content:function(){
							game.broadcastAll(function(player){
								...
								if(player.isTurnedOver()){
									if(player!=game.me){
										...
										setTimeout(function(){
											player.style.transform='none';
											player.node.avatar.setBackgroundImage('extension/扩展ol/'+player.name+'.jpg');
					...
					"NHHL_yiwei":{
						...
						content:function(){
							"step 0"
							player.chooseControl(['变为【决斗】','变为【过河拆桥】','cancel2']).set('ai',function(event){
								if(player.countCards('h',{name:'sha'})>1&&player.hp>1) return '变为【决斗】';
								return 'cancel2';
							});
							"step 1"
							if(result.control=='变为【决斗】'){
						...
						content:function(){
							...
							if(result.bool){
								player.chooseTarget('请选择目标',function(card,player,target){
									...
									if(target.isTurnedOver()) return ai.get.attitude(player,target);
									return -ai.get.attitude(player,target);
								};
							}else{
								event.finish();
				...
				translate:{
					...
					"NHHL_zhouji":"周姬",
					"NHHL_yiwei":"义为",
					"NHHL_yiwei_info":"当你使用【杀】前，你可选择一项：1.该【杀】变为【决斗】；2.该【杀】变为【过河拆桥】，以此法转化的牌计入【杀】的使用次数且无法被【无懈可击】响应",
					"NHHL_shouling":"守陵",
					"NHHL_shouling_info":"你可将一张红色基本牌当作【杀】【闪】【桃】使用或打出，你每转化一张【桃】，你摸一张牌",
					"NHHL_renqin":"认亲",
					"NHHL_renqin_info":"出牌阶段限一次，你可交给一名其他角色一张牌，然后展示其一张手牌：若该牌为红色，你摸一张牌并获得技能“武圣”直到回合结束",
					"NHHL_huawu":"花武",
					...
					"NHHL_chanxian":"谗陷",
					"NHHL_chanxian_info":"摸牌阶段结束时，你可弃置一张手牌并指定一名其他角色，然后该角色依次获得你与你的下家各一张牌，然后将其武将牌翻面",
					"NHHL_dusi":"毒肆",
