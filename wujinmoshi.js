window.func = function (lib, game, ui, get, ai, _status) {
	...
	game.WJchangeCharacter = function () {
		...
		var WJchangeCharacterYES = ui.create.div('.menubutton.large', '<span style="cursor:pointer;">确认</span>',
		...
		lib.brawl.wujinmoshi = {
			...
			showcase: function (init) {
				if (lib.config.wujinmoshiYZR != true) {
					...
					var wujinmoshichooseCharacter1 = ui.create.div('.menubutton.large', '<span style="cursor:pointer;">选择<br>武将</span>',
					...
					function () {
						...
						var wujinmoshiYES = ui.create.div('.menubutton.large', '<span style="cursor:pointer;">确认</span>',
					...
					wujinmoshiDSDJC.style.top = '185px';
					...
					wujinmoshiDamageE.style.top = '205px';
					...
					wujinmoshiMaxHpE.style.top = '225px';
					...
					wujinmoshiMaxHandCardE.style.top = '245px';
					...
					wujinmoshiDrawE.style.top = '265px';
					...
					wujinmoshiRecoverE.style.top = '285px';
					...
					wujinmoshiESkillE.style.top = '305px';
					...
					setInterval(function () {
						if (lib.config.wujinmoshiESkillE == undefined) {
							...
							wujinmoshiESkillE.innerHTML = '额外技能：' + "<div style='white-space:nowrap;text-overflow:ellipsis;overflow:hidden;' title='"+lib.translate[lib.config.wujinmoshiESkillE+'_info']+"'>"+lib.translate[lib.config.wujinmoshiESkillE]+"（鼠标悬停可以查看技能简介）</div>";
						};
					},
					100);
					if (lib.config.wujinmoshiP == undefined) wujinmoshiESkillE.hide();

					var wujinmoshiGainLHS = ui.create.div();
					wujinmoshiGainLHS.style.left = '0px';
					wujinmoshiGainLHS.style.top = '145px';
					this.appendChild(wujinmoshiGainLHS);
					setInterval(function () {
						var num=0.05+lib.config.wujinmoshiDXB/150;
						if(num>0.45) num=0.45;
						num=num*10000;
						num=Math.floor(num);
						num=num/100;
						wujinmoshiGainLHS.innerHTML = '<br>获胜后得到灵魂碎片概率：'+num+'%';
					},
					100);
					if (lib.config.wujinmoshiP == undefined) wujinmoshiGainLHS.hide();

					var wujinmoshiRE = ui.create.div('.menubutton.large', '<span style="cursor:pointer;">重置</span>',
					...
					var wujinmoshiBAG = ui.create.div('.menubutton.large', '<span style="cursor:pointer;">背包</span>',

					function () {
						wujinmoshiBAG.delete();
						var wujinmoshiBAGJM;
						var wujinmoshiBAGQX = ui.create.div('.menubutton.large', '<span style="cursor:pointer;">关闭</span>',
						function () {
							wujinmoshiBAGQX.delete();
							wujinmoshiBAGJM.delete();
							delete game.kzol_wujinmoshiBAGJM;
						...
						var wujinmoshiHJK = ui.create.div('', '<span style="cursor:pointer;">换将卡<br>' + lib.config.wujinmoshiHJK + '张</span>',
						...
						var wujinmoshiXRK = ui.create.div('', '<span style="cursor:pointer;">削弱卡<br>' + lib.config.wujinmoshiXRK + '张</span>',
						...
						game.kzol_wujinmoshiBAGJM=wujinmoshiBAGJM;
					});
					wujinmoshiBAG.style.left = '500px';
					wujinmoshiBAG.style.top = '50px';
					this.appendChild(wujinmoshiBAG);

					var wujinmoshiJS = ui.create.div('.menubutton.large', '<span style="cursor:pointer;">规则</span>',
					...
					function () {
						...
						var wujinmoshiJSQX = ui.create.div('.menubutton.round', '×',
			...
			content: {
				gameStart: function () {
					...
					var characterAi1=characterAi.randomGet();
					game.me.next.init(characterAi1);
					_status.kzol_characterAi1=characterAi1;
					_status.kzol_characterAi2=game.me.next;
					...
					/*game.addPlayer = function (position,character,character2) {
						alert('增加角色，违反无尽模式规则，重新载入游戏');
						game.reload();
					};
					var addFellow=game.addFellow;
					game.addFellow = function (position,character,animation) {
						if(_status.addingShibing==true){
							return addFellow(position,character,animation);
						}else{
							alert('增加角色，违反无尽模式规则，重新载入游戏');
							game.reload();
						};
			...
			init: function () {
				game.identityVideoName='无尽模式（乱斗）';
				...
				lib.onover.push(function(result){
					if(result==true){
						var num=0.05+lib.config.wujinmoshiDXB/150;
						if(num>0.45) num=0.45;
						if (Math.random() <=num&&game.gainItem!=undefined&&lib.kzol_bag!=undefined&&lib.kzol_bag['kzol_bw_lhs_'+_status.kzol_characterAi1]!=undefined) game.gainItem('kzol_bw_lhs_'+_status.kzol_characterAi1,1);
						if (Math.random() <= 0.01) {
				...
				lib.skill._wujinmoshiRE = {
					...
					content: function () {
						if(player == game.me){
							...
							game.wujinmoshiRE();
						}else if(player==_status.kzol_characterAi2){
				...
				lib.skill.wujinmoshiRE2 = {
					createDialog: function (player, target, onlylist) {
						var list = lib.character[_status.kzol_characterAi1][3];
				...
				lib.skill._wujinmoshiDamageE = {
					...
					filter: function (event, player) {
						return player != game.me&&player.扩展ol_owner==undefined;
					},
					forced: true,
					content: function () {
				...
				lib.skill._wujinmoshiMaxHandCardE = {
					mod: {
						maxHandcard: function (player, num) {
							if (player != game.me&&player.扩展ol_owner==undefined) return num + Math.floor(lib.config.wujinmoshiDXB / 5) - lib.config.wujinmoshiMaxHandCardI;
							return num;
						},
					},
				};
				...
				if (game.kzol_wujinmoshiBAGJM){
					game.kzol_wujinmoshiBAGJM.delete();
					delete game.kzol_wujinmoshiBAGJM;
				};
			},
		};
