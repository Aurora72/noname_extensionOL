window.func=function(lib,game,ui,get,ai,_status){
		...
		game.import('character',function(){
			var yl={
				...
				connect:true,
				character:{
					"yin_严颜":["male","shu",4,["yl_拒战"],['die_audio']],
					"yin_王平":["male","shu",4,["yl_飞军","yl_兵略"],['die_audio']],
					"yin_陆绩":["male","wu",3,["yl_怀橘","yl_遗礼","yl_整论"],['die_audio']],
					"yin_孙亮":["male","wu",3,["yl_溃诛","yl_掣政","yl_立军"],['zhu','die_audio']],
					"yin_许攸":["male","qun",3,["yl_成略","yl_恃才","yl_寸目"],['die_audio']],
					"yin_卢植":["male","qun",3,["yl_明任","yl_贞良"],['die_audio']],
					"yin_神刘备":["male","shen",6,["yl_龙怒","yl_结营"],[]],
					"yin_神陆逊":["male","shen",4,["yl_军略","yl_摧克","yl_绽火"],[]],
					"yin_蒯良蒯越":["male","wei",3,["yl_荐降","yl_审时"],['die_audio']],
					"lei_毋丘俭":["male","wei",4,["yl_徵荣","yl_鸿举"],[]],
					"lei_郝昭":["male","wei",4,["yl_镇骨"],['die_audio']],
					"lei_诸葛瞻":["male","shu",3,["yl_罪论","yl_父荫"],['die_audio']],
					"lei_周妃":["female","wu",3,["yl_良姻","yl_箜声"],[]],
					"lei_陆抗":["male","wu",4,["yl_谦节","yl_决堰","yl_破势"],[]],
					"lei_袁术":["male","qun",4,["yl_庸肆","yl_伪帝"],['die_audio']],
					"lei_张绣":["male","qun",4,["yl_雄乱","yl_从谏"],['die_audio']],
					"lei_陈到":["male","shu",4,["yl_往烈"],['die_audio']],
				...
				skill:{
						...
						subSkill:{
							...
							'2':{
								...
								content:function(){
									...
									player.storage.yl_拒战2=trigger.targets;
								},
							},
						},
					},
					...
					"_yl_拒战":{
						mod:{
							targetEnabled:function(card,player,target){
								if(player.storage.yl_拒战2!=undefined&&player.storage.yl_拒战2.contains(target)) return false;
							},
						},
						trigger:{
							player:'phaseAfter'
						...
						filter:function (event,player){
							return player.storage.yl_拒战==true||(player.storage.yl_拒战2!=undefined&&player.storage.yl_拒战2!=[]);
						},
						forced:true,
						popup:false,
						content:function(){
							if(player.storage.yl_拒战==true) delete player.storage.yl_拒战;
							if(player.storage.yl_拒战2!=undefined&&player.storage.yl_拒战2!=[]) player.storage.yl_拒战2=[];
					...
					"yl_飞军":{
						...
						intro:{
							content:function(storage){
								if(storage==undefined||(storage!=undefined&&storage.length==0)) return '未发动';
								var str='';
						...
						content:function(){
							...
							if(result.bool){
								...
								if(player.hasSkill('yl_兵略')&&!player.storage.yl_飞军.contains(event.target)) event.bool=true;
							...
							'1':{
								...
								filter:function (event,player){
									...
									for(var i=0;i<game.players.length;i++){
										...
										for(var i in pl.storage){
											if(get.itemtype(pl.storage[i])=='cards'){
												for(var j=0;j<pl.storage[i].length;j++){
													list.push(pl.storage[i][j]);
												};
											};
										};
									};
				...
				translate:{
					...
					"lei_张绣":"张绣",
					"yl_雄乱":"雄乱",
					"yl_雄乱_info":"限定技，出牌阶段，你可以废除你的判定区，然后指定一名其他角色，直到回合结束，你使用的牌无距离和次数限制，其不能使用和打出手牌",
					"yl_从谏":"从谏",
