window.func = function (lib, game, ui, get, ai, _status) {
	if(lib.kzol_all_character==undefined){
		lib.kzol_all_character={};
		for(var i in lib.characterPack){
			for(var j in lib.characterPack[i]){
				lib.kzol_all_character[j]=lib.characterPack[i][j];
			};
		};
	};
	var jilueduijueCharacter1 = [];
	for(var i in lib.characterPack){
		if(lib.config['extension_扩展ol_ldmsxx_jldj']=='tb'){
			if(lib.config.characters.contains(i)){
				for(var j in lib.characterPack[i]){
					if (lib.characterPack[i][j][4]==undefined||
					(lib.characterPack[i][j][4]!=undefined&&
					!lib.characterPack[i][j][4].contains('forbidai')&&
					!lib.characterPack[i][j][4].contains('boss')&& 
					!lib.characterPack[i][j][4].contains('hiddenboss'))) jilueduijueCharacter1.push(j);
				};
			};
		}else{
			for(var j in lib.characterPack[i]){
	...
	if (lib.config.gameMeHasPlayer1 == undefined && lib.config.gameMeHasPlayerA == undefined) {
		...
		game.saveConfig('gameMeHasPlayerHp1', lib.kzol_all_character[lib.config.jilueduijue1][2]);
		...
		lib.brawl.jilueduijue = {
				...
				if (lib.config.jilueduijueYZR != true) {
					...
					var jilueduijueJL = ui.create.div('.menubutton.large', '<span style="cursor:pointer;">记录</span>', function () {
						...
						var jilueduijueJLQX = ui.create.div('.menubutton.large', '<span style="cursor:pointer;">关闭</span>', function () {
							...
							delete game.kzol_jilueduijueJLJM;
						...
						game.kzol_jilueduijueJLJM=jilueduijueJLJM;
					});
			...
			init: function () {
				...
				lib.onover.push(function(result){
					if(result==true){
						if (lib.config.jilueduijueDXG == 1) {
							game.saveConfig('gameMeHasPlayer2', lib.config.jilueduijueE);
							game.saveConfig('gameMeHasPlayerHp2', lib.kzol_all_character[lib.config.gameMeHasPlayer2][2]);
						};
						if (lib.config.jilueduijueDXG == 2) {
							game.saveConfig('gameMeHasPlayer3', lib.config.jilueduijueE);
							game.saveConfig('gameMeHasPlayerHp3', lib.kzol_all_character[lib.config.gameMeHasPlayer3][2]);
						};
						if (lib.config.jilueduijueDXG == 3) {
							game.saveConfig('gameMeHasPlayer4', lib.config.jilueduijueE);
							game.saveConfig('gameMeHasPlayerHp4', lib.kzol_all_character[lib.config.gameMeHasPlayer4][2]);
						};
						if (lib.config.jilueduijueDXG == 4) {
							game.saveConfig('gameMeHasPlayer5', lib.config.jilueduijueE);
							game.saveConfig('gameMeHasPlayerHp5', lib.kzol_all_character[lib.config.gameMeHasPlayer5][2]);
						};
				...
				if (game.kzol_jilueduijueJLJM){
					game.kzol_jilueduijueJLJM.delete();
					delete game.kzol_jilueduijueJLJM;
				};
			},
		};
